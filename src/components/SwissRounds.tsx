...
export function SwissRounds({ matches, participants, onSubmitResults, currentRound, tournamentId, tournamentName }: SwissRoundsProps) {
    // Group matches by round
    const rounds = Array.from({ length: 3 }, (_, i) => i + 1);
                    ...
                    <div key={round} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {roundMatches.map(match => (
                                <Card key={match.id} className={match.isCompleted ? "opacity-75 bg-muted/20" : "border-primary/50"}>
                                    <CardHeader className="pb-3 flex flex-row items-center justify-between space-y-0">
                                        <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-3 flex-wrap">
                                            ...
                                            {(() => {
                                                ...
                                                const scale = playerCount === 3 ? "3/2/1" : "5/3/2/1";
                                                return <span className="text-xs text-muted-foreground/60 font-normal">{playerCount}p · {scale} pts</span>;
                                            })()}
                                            {match.isCompleted && <span className="text-green-500">Approved</span>}
                                            {match.isPendingReview && !match.isCompleted && <span className="text-yellow-500">Pending Review</span>}
                                        </CardTitle>
                                    </CardHeader>
                                                    ...
                                                    {(match.participantIds || []).map((pId, i) => {
                                                        ...
                                                        return (
                                                            <div key={i} className="flex items-center justify-between p-2 rounded-md bg-accent/50 text-sm">
                                                                <span>{p ? p.name : "Empty Seat"}</span>
                                                                {match.isCompleted && p && (
                                                                    <span className="font-bold text-primary">+{match.results[p.id] || 0} pts</span>
                                                                )}
                                                            </div>
                                                        );
                                                    })}
